...
页面依次引导载入、目标/补问、候选和下载。生成前不展示空报告和空下载区。
本轮 UX1 技术修复与用户使用验收分开记录，见 [UX1 记录](../records/DEMO_V1/UX1_USER_FLOW_FIX.md)。
...
2. **摘要与补问**：先查看系统名、组件/功能和可分析对象数量，再选择组件/功能。
   文件 hash、解析器/运行时版本、系统结构和来源保留在展开区。
   提交自然语言分析目标和工况后查看模型确认的目标。最多两轮集中补问；空回答不会新建事实或调用模型。
   点击“提交补充”发送已填写信息；空白会提示操作且不消耗补问轮次。
   无法确认时直接点击“按未知继续”，该操作也会提交已填写内容，仍缺少的信息保留 UNKNOWN。
   结构冲突、无效目标、解析错误不能绕过。进入可生成状态后，页面会提示点击“生成候选报告”。
...
“确认降级并生成候选报告”，仅依据已有资料推断，保留原 ERROR，不重查或冒充 NO_MATCH。
如果出现意外操作异常，页面会停止该会话的提交，并提供“新建分析会话”入口；
不会自动重放结果不明的请求。已知分析失败提供诊断下载，失败不表示系统没有失效。
输入拒绝时按错误提示检查/更换文件并重新载入，或用演示资料包核对流程；不会静默截断模型。
