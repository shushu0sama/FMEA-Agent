...
## 7. D0 后编号与路线澄清（2026-09-05）

用户指出原 MVP 路线与新 D1–D7 编号可能冲突。本次从 `740279c`、
`codex/demo-v1-spec-plan`、干净工作区核查，发现 Spec 已声明 Demo 选取 MVP-2/3 的受限能力，
但缺少逐项映射与正式阶段恢复说明，容易造成交接误读。

本次在 Spec 第 1.1 节补充映射、演示优先的排期调整和正式验收边界；
Plan 的 D7 在同一验收报告中登记 MVP-2/3 复用能力与差距；PROGRESS 和文档导航链接该解释。
不重编号原 MVP，不更改历史 D0 审查结论，不新增产品能力或修改 A01–A12 验收项。
上述补充未另行独立审核；此前 ACCEPTED 仍指第 5 节记载的 D0 审查范围。
D1–D7 仍未实施，下一步为 Demo V1 / D1。

LOCAL：5 份 Markdown 围栏、35 个本地链接目标和 3 个章节锚点检查通过；
`git diff --check` 通过。源码/测试/依赖无变化，本轮未重复运行 pytest/ruff/mypy，
基准与 gold 无变化；不把上一轮 223 项通过记录当成本轮执行结果。
